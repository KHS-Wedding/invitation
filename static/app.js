(() => {
    ...
    return `
      <div class="gallery-grid reveal" aria-label="웨딩 사진 갤러리">
        ${gallery.map((image, index) => `
          <button
            type="button"
            class="gallery-item protected-photo"
            data-gallery-index="${index}"
            aria-label="${escapeHtml(image.alt || `웨딩 사진 ${index + 1}`)} 크게 보기"
            draggable="false"
            style="${backgroundStyle(image.src)}"
          ></button>`).join('')}
      </div>`;
  }
  ...
  function renderMap() {
    ...
    return `
      <figure class="map-card reveal">
        <img src="${escapeHtml(images.map_image)}" alt="${escapeHtml(venue.name)} 약도" draggable="false" />
      </figure>`;
  }
  ...
  function render() {
    ...
    const coverMarkup = images.cover
      ? `<div class="hero-photo protected-photo" role="img" aria-label="${escapeHtml(images.cover_alt)}" draggable="false" style="${backgroundStyle(images.cover)}"></div>`
  ...
  function setupInteractions() {
    ...
    modal?.querySelector('.modal-prev')?.addEventListener('click', () => updateGalleryModal(currentGalleryIndex - 1));
    modal?.querySelector('.modal-next')?.addEventListener('click', () => updateGalleryModal(currentGalleryIndex + 1));
    ...
    modal?.addEventListener('pointerup', (event) => {
      ...
      updateGalleryModal(currentGalleryIndex + (distance < 0 ? 1 : -1));
    });
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (event.key === 'ArrowLeft') updateGalleryModal(currentGalleryIndex - 1);
      if (event.key === 'ArrowRight') updateGalleryModal(currentGalleryIndex + 1);
    });
    ...
    // 개인 사진의 일반적인 우클릭 저장과 드래그를 막습니다.
    // 다만 공개 웹페이지 특성상 개발자 도구·원본 URL·화면 캡처까지 완전히 차단할 수는 없습니다.
