  ...
  function setupInteractions() {
    ...
    const preloadAdjacent = () => {
      ...
      [-2, -1, 0, 1, 2].forEach((offset) => {
        preloadGalleryImage(currentGalleryIndex + offset);
      });
    };
    ...
    const updateGalleryModal = (index, direction = 0, mode = 'normal') => {
      ...
      if (previewSource !== image.src) {
        modalPhoto.classList.add('is-preview-loading');
        preloadGalleryImage(currentGalleryIndex).then(() => {
          if (requestId !== modalImageRequestId) return;
          modalPhoto.style.backgroundImage = `url("${encodeURI(image.src).replaceAll('"', '%22')}")`;
          modalPhoto.classList.remove('is-preview-loading');
        });
      } else {
        modalPhoto.classList.remove('is-preview-loading');
      }
    ...
    if ('IntersectionObserver' in window) {
      const fullImageObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting) return;
          const index = Number(entry.target.dataset.galleryIndex || 0);
      ...
      document.querySelectorAll('.gallery-item').forEach((button) => {
        fullImageObserver.observe(button);
      });
    }
